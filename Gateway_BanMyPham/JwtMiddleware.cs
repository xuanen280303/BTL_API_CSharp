...
namespace GateWay
{
    public class JwtMiddleware
    {
        ...
        public JwtMiddleware(RequestDelegate next, IOptions<AppSettings> appSettings, IConfiguration configuration, IDatabaseHelper databaseHelper)
        {
            _next = next;
        ...
        public async Task GenerateToken(HttpContext context)
        {
            ...
            try
            {
                ...
                var MaNguoiDung = query.Rows[0]["MaNguoiDung"].ToString();
                var HoTen = query.Rows[0]["HoTen"].ToString();
                var TaiKhoan = query.Rows[0]["TaiKhoan"].ToString();
                var MatKhau = query.Rows[0]["MatKhau"].ToString();
                var LoaiQuyen = query.Rows[0]["LoaiQuyet"].ToString();
                ...
                var tokenDescriptor = new SecurityTokenDescriptor
                {
                    ...
                    new Claim(ClaimTypes.Role,LoaiQuyen),
                    new Claim(ClaimTypes.DenyOnlyWindowsDeviceGroup, MatKhau)
                    }),
                ...
                var response = new { MaNguoiDung = MaNguoiDung, HoTen = HoTen, TaiKhoan = TaiKhoan, Token = token };
